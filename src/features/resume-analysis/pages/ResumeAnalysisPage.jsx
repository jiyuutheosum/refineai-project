...
import { analyzeResume, resetAnalysis } from '../store/analysisSlice'
...
function ResumeAnalysisPage() {
  ...
  const { overallScore, overallFeedback, sectionFeedback, status, error } =
    useAppSelector((state) => state.analysis)

  // Get the raw File object from navigation state
  ...
  useEffect(() => {
    ...
    // If analysis already succeeded, don't redirect — just show results
    if (status === 'succeeded') return

    // If no file in navigation state but we have results, keep showing them
    if (!file && status === 'succeeded') return

    // Only redirect to upload if we have no resume AND no existing results
  ...
  }, [dispatch, currentResume, file, navigate, status])

  // Only block render if no resume AND no results
  if (!currentResume) return null
  if (!file && status !== 'succeeded') return null

  const analysisItems = sectionFeedback.length > 0
    ...
    : [
        { title: 'Contact Information', score: 0, description: 'Analyzing...' },
        { title: 'Professional Summary', score: 0, description: 'Analyzing...' },
        { title: 'Work Experience', score: 0, description: 'Analyzing...' },
        { title: 'Skills', score: 0, description: 'Analyzing...' },
      ]
      ...
      <main className="container mx-auto px-4 py-8 md:px-6 md:py-12 lg:px-8 lg:py-16">
        <div className="mx-auto max-w-6xl">
          <header className="mb-10 text-center">
            <p className="mb-3 text-sm font-medium text-primary">Resume Analysis</p>
            ...
            <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-muted-foreground md:text-lg">
              Review your resume score, identify improvement areas, and continue to the editor to refine your resume.
            </p>
          </header>
          ...
          <div className="mb-6 flex items-center gap-3 rounded-xl border bg-muted/40 px-5 py-3 text-sm text-muted-foreground">
            ...
            <span className="ml-auto">
              {currentResume.fileSize ? `${(currentResume.fileSize / 1024).toFixed(1)} KB` : ''}
            </span>
          </div>
            ...
            <div className="grid gap-6 md:grid-cols-[220px_1fr] md:items-center">
              <div className="flex justify-center">
                <div className="flex h-40 w-40 flex-col items-center justify-center rounded-full border-8 border-primary/20 bg-primary/5">
                  <span className="text-5xl font-bold text-primary">{overallScore || 0}</span>
              ...
              <div>
                ...
                <p className="mt-3 leading-7 text-muted-foreground">
                  {overallFeedback || 'Your resume has been analyzed. See the breakdown below.'}
                </p>
